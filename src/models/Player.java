...
public class Player extends Observable  {
	...
	private int playersFeuerSchaden = 20;
	private int playersEisSchaden=20;
	private int fireDmg=20;
	...
	public int getPlayersFeuerSchaden()
	{
		return this.playersFeuerSchaden;
	}
	
	public void setPlayersFeuerSchaden(int d)
	{
		this.playersFeuerSchaden=d;
	}
		...
		for (int i = 0; i < movObj.getRskills().size(); i++) {
			...
			
			{
				if ((movObj.getRskills().get(i).getOwner() != this)
					...
					else if(movObj.getRskills().get(i).getSkillTyp().equals("eis"))
					{
						this.takeIceDamage(movObj.getRskills().get(i).getDmg());
					}
	...
	{
		if(this.getIceArmor()>=dmg)
		{
			this.setIceArmor(this.getIceArmor()-dmg);
		}
		else 
		{
			dmg= dmg-this.getIceArmor();
			this.setIceArmor(0);
			if ((healthpoints - dmg) > 0)
				this.healthpoints = healthpoints - dmg;
			else
	...
	public void shot() {
		...
		else if (this.getTypedforSkill() == 3) {
			if (this.getManaPoints() > this.getManaCost()) {
				this.takeManaDmg();
			

				iceball = new IceBall(this.getPosX(), this.getPosY(), this.pf,
						this);
				iceball.setSkillTyp("eis");
				iceball.setRange(this.getPlayersBallRange());
				switch (direction) {
				case 0:
					iceball.setSpeedThread(this.getSpeed() - 2);
					iceball.setPos(this.getPosX() + 10, this.getPosY() + 40);
					iceball.setSpeedY(1);

					break;
				case 1:
					iceball.setSpeedThread(this.getSpeed() - 2);
					iceball.setPos(this.getPosX() - 20, this.getPosY() + 20);
					iceball.setSpeedX(-1);
					break;
				case 2:
					iceball.setSpeedThread(this.getSpeed() - 2);
					iceball.setPos(this.getPosX() + 10, this.getPosY() - 20);
					iceball.setSpeedY(-1);
					break;
				case 3:
					iceball.setSpeedThread(this.getSpeed() - 2);
					iceball.setPos(this.getPosX() + 40, this.getPosY() + 20);
					iceball.setSpeedX(1);
					break;
				}

				this.movObj.addRskill(iceball);
	...
	{
		return this.fireArmor;
	}
	public void setFireArmor(int fArmor)
	{
		this.fireArmor=fArmor;
	}
	...
	public void setEnTyp(String enTyp) {
		this.enTyp = enTyp;
	}
	...
	public int getPlayersEisSchaden() {
		return playersEisSchaden;
	}

	public void setPlayersEisSchaden(int playersEisSchaden) {
		this.playersEisSchaden = playersEisSchaden;
	}
